...
void SimpleSerial::ReOpenPort(char* com_port, DWORD COM_BAUD_RATE)
{
	ClearComm();
	CloseSerialPort();
	CreateSerialPort(com_port, COM_BAUD_RATE);
...
void SimpleSerial::CreateSerialPort(char* com_port, DWORD COM_BAUD_RATE)
{
	connected_ = false;
	...
	SetupComm(io_handler_, inputBufferSize, outputBufferSize);
