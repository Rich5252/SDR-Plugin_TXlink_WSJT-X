 ...
 */
 ...
 // REFACTORED CODE THAT TAKES FREQ*100 as input to give max resolution
 //  Min freq step is 0.0419 Hz
...
public:
  ...
  void reset()
  {
    ...
    setClock(0);
    // To avoid possible false configuration settings, immediately set the frequency to 1 Hz (100 in new units)
    setFrequency(100, false);
  }
  ...
  // NOTE: REFACTORED implementation to take freq*100 as input to generate higher resolution
  // Freq resolution step is 0.0419 Hz so error +/- 0.021Hz
  uint32_t frequencyDelta(uint32_t freq_x100) const
  {
#if     defined(DDS_MAX_PRECISION)
    return ((uint64_t)freq_x100 * reciprocal / 100 + 0x80000000UL) >> 32;
#else
    // Applying the 1/100 factor to the reciprocal maintains the precision
    // and keeps the rest of the original code structure simple.
    // Note: The original had a fractional part (reciprocal/16) for rounding.
    // We divide that by 100 as well.
    return ((uint64_t)freq_x100 * (reciprocal / 100) + (reciprocal/16/100)) >> shift;
#endif
  }


  // NOTE: freq is now freq_x100 (in units of 0.01 Hz)
  void setFrequency(uint32_t freq_x100, bool PowerUp)
  {
    setDelta(frequencyDelta(freq_x100), PowerUp);
  }

  void setDelta(uint32_t delta_word, bool PowerUp)
  {
